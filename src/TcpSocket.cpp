...
int TcpSocket::connect(const std::string& ipaddr, std::uint16_t port, std::chrono::milliseconds timeout)
{
  int iResult = 0;
...
#ifdef _WIN32
  long timeoutUSeconds = static_cast<long>(std::chrono::duration_cast<std::chrono::microseconds>(timeout).count());
  long timeoutSeconds  = timeoutUSeconds / 1000000L;
  timeoutUSeconds -= timeoutSeconds * 1000000;
#else
  suseconds_t timeoutUSeconds =
    static_cast<suseconds_t>(std::chrono::duration_cast<std::chrono::microseconds>(timeout).count());
  time_t timeoutSeconds = static_cast<time_t>(timeoutUSeconds / 1000000);
  timeoutUSeconds -= timeoutSeconds * 1000000;
#endif
  ...
  const DWORD timeoutMs = static_cast<DWORD>(std::chrono::duration_cast<std::chrono::milliseconds>(timeout).count());
