  ...
  : m_pData(std::make_shared<SocketData>())
{
  ...
  *reinterpret_cast<uint32_t*>(&m_pData->remoteAddr.sin_addr) = htonl(string2addr(remoteIp));
  ...
  if (prefix > 32)
  {
    std::cerr << "ERR: invalid prefix" << std::endl;
    return;
  }
  int shift = 32 - prefix;
  ...
  {
    std::cerr << "WARN: remote in different network" << std::endl;
  }
  ...
  if (!m_init)
  {
    std::cerr << "ERR: WSAStartup failed" << std::endl;
  }
#else
  if (m_broadcast)
  {
    ...
    {
      std::cerr << "ERR: open rxsock failed" << std::endl;
      return;
    }
  }
#endif
  ...
  {
    std::cerr << "ERR: open socket failed" << std::endl;
    return;
  }
  ...
  if (setsockopt(m_pData->socket, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<char*>(&option), sizeof(option)) < 0)
    std::cerr << "WARN: set SO_RCVTIMEO failed" << std::endl;
#else
  timeval timeout;
  ...
  if (m_broadcast)
  {
    ...
    if (setsockopt(m_pData->socket, SOL_SOCKET, SO_BROADCAST, reinterpret_cast<char*>(&option), sizeof(option)) < 0)
      std::cerr << "WARN: set SO_BROADCAST failed" << std::endl;
#ifndef _WIN32
  ...
  if (bind(m_pData->socket, reinterpret_cast<sockaddr*>(&m_pData->localAddr), sizeof(m_pData->localAddr)) < 0)
    std::cerr << "WARN: set bind failed" << std::endl;
#ifndef _WIN32
  if (m_broadcast)
  {
...
uint32_t NetLink::string2addr(std::string const& ip)
{
  ...
  {
    std::cerr << "ERR: invalid remote ip" << std::endl;
    return 0;
  }
  return ntohl(addr);
}
...
int NetLink::read(std::vector<uint8_t>& buffer)
{
  if (m_pData->socket == INVALID_SOCKET)
    return -1;
  int len = recv(m_pData->socket, reinterpret_cast<char*>(buffer.data()), static_cast<unsigned short>(buffer.capacity()), 0); // mtu = 9000 for jumbo frames should perfectly fit into unsigned short
#ifndef _WIN32
  if (len <= 0)
  {
    if (m_pData->rxsock == INVALID_SOCKET)
      return -1;
    len = recv(m_pData->rxsock, reinterpret_cast<char*>(buffer.data()), static_cast<unsigned short>(buffer.capacity()), 0); // mtu = 9000 for jumbo frames should perfectly fit into unsigned short
  }
#endif
  if (len > 0)
